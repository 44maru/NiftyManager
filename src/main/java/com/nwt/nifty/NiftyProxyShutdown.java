...
public class NiftyProxyShutdown extends NiftyProxyManager {
	...
	public void stopServer(String[] srvInfo, int lineNum) {
		...
		if (result.getReservations() != null) {
			checkStatusAndStopServer(srvInfo, client, result);
		} else {
	...
	private void checkStatusAndStopServer(String[] srvInfo, NiftyServerClient client, DescribeInstancesResult result) {
		...
		if (state.getCode() == SRV_STATE_RUNNING) {
			...
			if (stopInstances(srvInfo, client)) {
				...
				checkInstanceStopped(srvInfo[INDEX_INSTANCE_ID], client); 
			} else {
	...
	private boolean retryWaiting(String instanceId, NiftyServerClient client, DescribeInstancesRequest request)
		...
		while (true) {
			...
			if (end.getTime() - start.getTime() > 60000) {
				log.error("InstanceID {} => 1分経過してもサーバが停止しなかったため、停止監視をあきらめます。サーバのステータス[{}]",
						instanceId, state);
				counter.addErrorCnt();
				return false;
			}
